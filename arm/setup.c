...
/*
reading notes:
The code runs on the boot CPU itself, so read_cpuid_mpidr() returns the boot CPU's hardware ID.

cpu_logical_map: 
Array that translates logical CPU numbers (used by kernel) to physical IDs.
Ensures the boot CPU is always logical CPU 0 (kernel convention).
Other CPUs are mapped linearly unless they collide with the boot CPU.

Hardware vs. Software Abstraction:
Physical CPUs	/ Logical CPUs
Represent actual hardware cores/threads/ Kernel's abstracted view of CPUs
Tied to hardware IDs (e.g., ARM's MPIDR)/	Sequential numbering (0 to nr_cpu_ids-1)
May have discontinuous/non-sequential IDs/	Always contiguous for efficient kernel operation

The function set_my_cpu_offset(0) plays a critical role in early ARM kernel initialization by resetting the CPU's per-CPU data offset to zero during boot.
Here's a detailed breakdown of its purpose and implications:
	Core Function
		1.Clears TPIDRPRW Register
		Writes 0 to ARM's Thread-PID Privileged-Only Register (TPIDRPRW, accessed via c13 coprocessor).
		This register typically stores the base address for per-CPU variables.
		2.Prevents Early Boot Crashes
		Ensures any accidental access to per-CPU variables (before proper initialization) defaults to offset 0, avoiding wild pointer dereferences.

	Why This is Needed?
	Problem Scenario:
		1.During very early boot, the kernel accesses some per-CPU variables (e.g., in lockdep or printk) before:
			Per-CPU areas are allocated.
			SMP CPU numbering is finalized.
		2.Without this reset, the CPU might use stale/garbage values in TPIDRPRW, causing crashes.
		Solution
	Forcing TPIDRPRW=0 makes early per-CPU accesses fall back to a common base address (usually the first CPU's data).
*/
